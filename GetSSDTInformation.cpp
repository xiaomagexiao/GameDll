...
BOOL GetSSDTInformation::FindFirst(OUT char *pServiceName, IN int &nServiceID)
{
    try
    {
        ...
        {
            OutputDebugStringA("pServiceName is NULL!\r\n") ;
            return FALSE ;
        }
        ...
        {
            OutputDebugStringA("OpenFileAndCreateMap fail!\r\n") ;
            return FALSE ;
        }
        ...
        if (IMAGE_DOS_SIGNATURE != pDos->e_magic)
        {
            ...
            FindClose() ;
            OutputDebugStringA("不是一个有效PE文件,DOS头不对!\r\n") ;
            return FALSE ;
        }
        ...
        {
            FindClose() ;
            OutputDebugStringA("不是一个有效PE文件,指向不正确的nt头") ;
            return FALSE ;
        }
        ...
        {
            FindClose() ;
            OutputDebugStringA("不是一个有效PE文件,NT头不对") ;
            return FALSE ;
        }
        ...
        {
            FindClose() ;
            OutputDebugStringA("导出表VirtualAddress错误了吧!\r\n") ;
            return FALSE ;
        }
        ...
        {
            FindClose() ;
            OutputDebugStringA("new IMAGE_SECTION_HEADER failed!\r\n") ;
            return FALSE ;
        }
...
BOOL GetSSDTInformation::OpenFileAndCreateMap(void)
{
    ...
    if (0 == GetWindowsDirectoryA(szWindowsDir, nSize))
    {
        dwError = GetLastError() ;
        OutputDebugStringA("GetWindowsDirectory failed!\r\n") ;
        return FALSE ;
    }
    ...
    m_hFile = CreateFileA(szWindowsDir,GENERIC_READ,
                            FILE_SHARE_READ,
    ...
    {
        dwError = GetLastError() ;
        OutputDebugStringA("CreateFileMapping failed!\r\n") ;
        // 如果出错了,再把打开的文件关掉
        FindClose() ;
        return FALSE ;
    }
...
int GetSSDTInformation::initSSDTFunctionName()
{
	char szServiceName[MAXBYTE] = {0} ;
	int  nServiceID = 0 ;
	int nCount = 0 ;
	if (TRUE == this->FindFirst(szServiceName, nServiceID))
	{
		OutputDbgInfo("[!] FunName: %-50s ServiceID: %04X", szServiceName, nServiceID) ;
		memcpy(functionArr[nServiceID],szServiceName,MAXBYTE-1);
		while (TRUE == this->FindNext(szServiceName, nServiceID))
		{
			OutputDbgInfo("[!] FunName: %-50s ServiceID: %04X", szServiceName, nServiceID) ;
			memcpy(functionArr[nServiceID],szServiceName,MAXBYTE-1);
			++nCount ;
	...
	OutputDbgInfo("SSDT Service Function Count: %d\r\n", nCount) ;
...
void GetSSDTInformation::GetSSDTNameByIndex(IN char *name, int index)
{
	
	if (strlen(functionArr[0])==0)
	{
		initSSDTFunctionName();
	}
	if (NULL == name || index>totalCount)
	{
		return;
	}
	char *p = functionArr[index];
	p[0] = 'Z';
	p[1] = 'w';
	memcpy(name, p, 50);
}